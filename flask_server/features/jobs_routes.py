...
def fetch_jobs_route_handler():
    try:
        ...
        #country = request.args.get('country', 'in') # Default to US if not specified
        country = 'in'
        
        app_id = current_app.config.get('ADZUNA_APP_ID')
        app_key = current_app.config.get('ADZUNA_APP_KEY')
 
        current_app.logger.info(f"Fetching Adzuna jobs: kw='{keywords}', loc='{location}', page={page}, limit={limit}, country={country}")
